<?php
	...
	$nameErr = $liberianidErr = $passwordErr = $confirmpasswordErr = $departmentErr= $usertypeErr= "";
	$joiningdateErr = $genderErr = $religionErr = $dobErr = $phoneErr = $emailErr = "";

	$name = $liberianid = $password = $usertype = $department = $joiningdate = $gender = $religion = $dob = $phone = $email = "";
	...
	if($_SERVER['REQUEST_METHOD'] === "POST")
	{
		...
		if (empty($_POST['liberianid'])) {
			$liberianidErr = "Id is mandatory";
			$flag = true;
		}
		else if(strlen($_POST['liberianid']) > 10) {
			$liberianidErr = "Maximum length of id is 10";
			$flag = true;
		}
		else if(is_numeric($_POST['liberianid'])) {
			$liberianid = validate_input($_POST['liberianid']);
		}
		else
		{
			$liberianidErr = "Id is consist of numbers";
			$flag = true;
		}
		...
		if(!$flag)
		{
			...
			$res = addLiberian($name, $liberianid, $password, $usertype, $department, $joiningdate, $gender, $religion, $dob, $phone, $email);
			if($res)
			{
			...
			<fieldset style="height:500px;width:px">
				...
				<fieldset>
					<table>
						...
						<tr>
							<td><label for="liberianid">Id<span style="color: red;">*</span></label></td>
							<td><input type="text" name="liberianid" id="liberianid"></td>
							<td><span style="color: red;" id="liberianidErr"><?php echo $liberianidErr; ?></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><input type="text" name="usertype" id="usertype" value="librarian" readonly=""></td>
						...
						<tr>
							<td><label for="joiningdate">Joining Date<span style="color: red;">*</span>: </label></td>
							<td><input type="date" name="joiningdate" id="joiningdate"></td>
							<td><span style="color: red;" id="joiningdateErr"><?php echo $joiningdateErr; ?></span></td>
						</tr>
						<tr>
