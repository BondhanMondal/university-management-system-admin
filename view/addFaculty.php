<?php
	...
	$nameErr = $facultyidErr = $passwordErr = $confirmpasswordErr = $departmentErr= $usertypeErr= "";
	$joiningdateErr = $genderErr = $religionErr = $dobErr = $phoneErr = $emailErr = "";

	$name = $facultyid = $password = $usertype = $department = $joiningdate = $gender = $religion = $dob = $phone = $email = "";
	...
	if($_SERVER['REQUEST_METHOD'] === "POST")
	{
		...
		if(!$flag)
		{
			...
			$res = addFaculty($name, $facultyid, $password, $usertype, $department, $joiningdate, $gender, $religion, $dob, $phone, $email);
			if($res)
			{
			...
			<fieldset style="height:500px;width:px">
				...
				<fieldset>
					<table>
						...
						<tr>
							<td><label for="facultyid">Id<span style="color: red;">*</span></label></td>
							<td><input type="text" name="facultyid" id="facultyid"></td>
							<td><span style="color: red;" id="facultyidErr"><?php echo $facultyidErr; ?></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><input type="text" name="usertype" id="usertype" value="faculty" readonly=""></td>
